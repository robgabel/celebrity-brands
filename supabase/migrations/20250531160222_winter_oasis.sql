/*
  # Add news feedback functionality
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_news_feedback_user_id') THEN
    CREATE INDEX idx_news_feedback_user_id ON news_feedback(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_news_feedback_brand_id') THEN
    CREATE INDEX idx_news_feedback_brand_id ON news_feedback(brand_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_news_feedback_url') THEN
